...
def pass_dependency(target, fuzz_data_logger, session, node, edge, *args, **kwargs):
    ...
    while True:
        try:
            ...
            if(len(banner)==0):
                ...
                break
            banner = bytes.decode(banner)
            if("\r\n" in banner):
                depend = banner.split("\r\n")[-1]
            # print (banner[0]>='0' and banner[0]<='9')
            if(banner[0]>='0' and banner[0]<='9'):
                depend=banner
        except:
            break
    # print (depend+"!!!!!!!!!!!!!!!!!!!!")
    node.names["depend"]._value = depend
    # print(node.render())
    # target.close()
    # session._open_connection_keep_trying(target)
    # target.open()

def change_port(target, fuzz_data_logger, session, *args, **kwargs):
    time.sleep(target.interval)
    banner = ""
    try:
        banner = target.recv(10000)
    except:
        logger.info("target too busy!!!")
        target.vmcontrol.restart_target()
    if str(banner)[0:10] == "HTTP/1.1 5":
    # if re.match(b"HTTP/1.1 5", banner):
        # restart_firm()
        logger.debug("Bad Response")
        # session.fuzz_node.mu
        target.vmcontrol.restart_target()
    while True:
        banner = target.recv(100000)
        if len(banner) == 0:
            break
    target.close()
    session._open_connection_keep_trying(target)
...
class Main_fuzzer:
    def __init__(self, firm_dir, pcap_dir):
        ...
        if "LOGIN" not in sections:
            logger.error("There is no login pcap")
            return False
        self.has_depend = "DEPENDENCY" in sections
        ...
        if self.has_depend:
            s_initialize("LOGIN")
            s_initialize("DEPENDENCY")
            s_initialize("HTTP")
            self.session.connect(s_get("LOGIN"))
    ...
    def fuzz_login(self):
        ...
        login_file = self.cf_read.options("LOGIN")[0]
        if re.match("POST", login_file):
            login_file = self.cf_read.get("LOGIN", login_file)
            with open(os.path.join(self.pcap_dir, "PCAPS", login_file), "r") as f:
        ...
        logger.info("Now fuzzing LOGIN node!")
        self.session.fuzz()
        logger.info("Cleaning up LOGIN node!")
        self.cleanup_node(s_get("LOGIN"))
    ...
    def fuzz_https(self):
        ...
        login_file = self.cf_read.options("LOGIN")[0]
        if re.match("POST", self.cf_read.get("LOGIN", login_file)):
            login_file = self.cf_read.get("LOGIN", login_file)
        # print(login_file)
        self.content.remove(login_file)
        with open(os.path.join(self.pcap_dir, "PCAPS", login_file), "r") as f:
            content = json.loads(f.read())
        s_switch("LOGIN")
        s_static(content["Header"]["method"] + " " + content["Header"]["path"] + " " + content["Header"]["version"] + "\r\n")
        for key, value in content["Header"].items():
            if key not in ["method", "path", "version"]:
                s_static(key)
                s_static(": ")
                s_static(value)
                s_static("\r\n")
        ...
        if self.has_depend:
            self.content.remove(self.cf_read.options("DEPENDENCY")[0])
            self.session.connect(s_get("LOGIN"), s_get("DEPENDENCY"), callback=change_port)
            self.session.connect(s_get("DEPENDENCY"), s_get("HTTP"), callback=pass_dependency)     
            logger.info("Loading depend file")
            depend_file = self.cf_read.options("DEPENDENCY")[0]
            with open(os.path.join(self.pcap_dir, "PCAPS", depend_file), "r") as f:
                content = json.loads(f.read())
            s_switch("DEPENDENCY")
            s_static(content["Header"]["method"] + " " + content["Header"]["path"].split("?")[0] + "?")
            num = 0
            ...
            self.session.connect(s_get("LOGIN"), s_get("HTTP"), callback=change_port)
    ...
    def fuzz_depend(self):      
        ...
        logger.info("Now fuzzing DEPENDENCY node!")
        self.session.fuzz()
        logger.info("Cleaning up DEPENDENCY node!")
        self.cleanup_node(s_get("DEPENDENCY"))
